    ...
    for (unsigned int t = 0; t < time_steps; t++){
        ...
        */
        ...
        for (unsigned int i = 0; i < map_size; ++i) {
            ...
            float p=motion_model(pseudo_position,  movement_per_timestep, priors,
                   map_size,  control_stdev);
            //TODO: get the motion model probability for each x position
            std::vector<float>ranges= pseudo_range_estimator(landmark_positions,
                                          pseudo_position);

            //TODO: get pseudo ranges
            p *= observation_model(landmark_positions, observations,
                        ranges,  distance_max,
                         observation_stdev);
